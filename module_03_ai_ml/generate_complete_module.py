"""
Generate ALL lessons 28-36 for Module 3: AI/ML in Finance
...
# Dictionary of all lessons
lessons = {}

# LESSON 28: Classification
lessons['lesson_28_classification.tex'] = PREAMBLE + r"""
...
\begin{frame}{Example: Loan Default Prediction}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Problem:}
\begin{itemize}
...
\begin{frame}{Making Predictions: Decision Threshold}
\begin{columns}[T]
\column{0.48\textwidth}
...
\begin{frame}{Classification Metrics: Accuracy}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Accuracy:}
\[
...
\begin{frame}{Application: Credit Scoring}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Problem:}
\begin{itemize}
...
for filename, content in lessons.items():
    ...
    count += 1
    print(f"Created ({count}/9): {filepath.name}")

print(f"\nCompleted {count} lessons. Total progress: 28/36 lessons.")
