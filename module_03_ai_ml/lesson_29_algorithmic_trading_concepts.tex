...
\begin{frame}
\titlepage
\bottomnote{Algorithmic trading: where finance meets computer science.}
\end{frame}
...
\begin{frame}[t]{What is Algorithmic Trading?}
\begin{columns}[T]
...
\begin{frame}[t]{Types of Strategies}
\begin{columns}[T]
...
\begin{frame}[t]{Backtesting Framework}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Steps:}
\begin{enumerate}
...
\begin{frame}[t]{Momentum Strategy Performance}
\begin{center}
\includegraphics[width=0.60\textwidth]{figures/momentum_strategy/momentum_strategy.pdf}
\end{center}
\bottomnote{Momentum strategies buy winners and sell losers, exploiting short-term trends in asset prices.}
\end{frame}

\begin{frame}[t]{Trading Strategy Backtest Results}
\begin{center}
\includegraphics[width=0.60\textwidth]{figures/trading_strategy_backtest/trading_strategy_backtest.pdf}
\end{center}
\bottomnote{Backtesting reveals strategy performance metrics including returns, drawdowns, and Sharpe ratios.}
\end{frame}

\begin{frame}[t]{Order Types and Execution}
\begin{center}
\includegraphics[width=0.60\textwidth]{figures/order_types/order_types.pdf}
\end{center}
\bottomnote{Different order types serve different execution objectives: market orders for speed, limit orders for price control.}
\end{frame}

\begin{frame}[t]{Market Making Spread Dynamics}
\begin{center}
\includegraphics[width=0.60\textwidth]{figures/market_making_spread/market_making_spread.pdf}
\end{center}
\bottomnote{Market makers profit from bid-ask spreads while providing liquidity to the market.}
\end{frame}

\begin{frame}[t]{Data Leakage in Backtesting}
\begin{center}
\includegraphics[width=0.60\textwidth]{figures/data_leakage/data_leakage.pdf}
\end{center}
\bottomnote{Look-ahead bias occurs when future information leaks into historical simulations, inflating backtest results.}
\end{frame}
...
\bottomnote{Survivorship bias inflates backtest returns by excluding failed companies from historical data.}
\end{frame}
